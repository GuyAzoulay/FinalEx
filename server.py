    ...
    with open(filename, "rb") as file:
        ...
        while True:
            ...
            if not bytes_read:
                break
            if i<10:
                sockUDP.sendto(f"0{i}<->".encode() + bytes_read, client_address)
                dataList.append(f"0{i}".encode()+bytes_read)
                #print(len(f"{i}".encode() + bytes_read))
            else:
                sockUDP.sendto(f"{i}<->".encode() + bytes_read, client_address)
                dataList.append(f"{i}".encode()+bytes_read)
                #print(len(f"{i}".encode() + bytes_read))
            ...
            i = i+1
        #ackList.remove(ackList[5])
        while len(ackList) < len(dataList):

            if len(ackList) == len(dataList):
                sockUDP.close()
                # send -> Send all
                break
            for j in ackList:
                temp = ackList.index(j)
                if j != int(temp):
                    sockUDP.sendto(dataList[temp],client_address) ## sending again
                    ack, _ = sockUDP.recvfrom(10)
                    ack = ack[4:6]
                    if ack[0] == '0':
                        ack = int(ack[1])
                ...
                if private[0] == '/':
                    ...
                    if destName == 'download':
                        ...
                        if filename not in files:
                            ...
                            client.sendall(
                                f"/download {socket.gethostbyname(socket.gethostname())} {port} {destFileName} {os.path.getsize(filename)}".encode())
